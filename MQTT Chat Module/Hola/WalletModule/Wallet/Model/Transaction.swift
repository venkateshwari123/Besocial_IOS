...
struct Transaction: Codable{
    ...
    var withdrawid = ""
    var rechargemode = ""
    var currencySymbol = Utility.getWalletCurrenySymbol()
    var receiverImage = ""
    ...
    init(data: [String:Any]) {
        ...
        if let amount = data["fromAmount"] as? Int {
            self.amount = "\(amount)"
        }
        ...
        if let currencySymbol = data["currencySymbol"] as? String, currencySymbol != ""{
        ...
        if let fee = data["Fee"] as? Int{
            self.fee = "\(fee)"
        }
        
        if let fee = data["txn_fee"] as? String{
            self.fee = fee
        }
        
        if let fee = data["txn_fee"] as? Int{
            self.fee = "\(fee)"
        }
        ...
        if let txnid = data["txnid"] as? Double{
            self.txnid = "\(txnid)"
        }
        ...
        if let transactionId = data["transactionId"] as? Double{
            self.txnid = "\(transactionId)"
        }
        if let transactionId = data["transactionid"] as? Double{
            self.txnid = "\(transactionId)"
        ...
        /*
         Bug Name:- Getting null for some reason
         Fix Date:- 14/06/21
         Fix By  :- Jayaram G
         Description of Fix:- Added check for null data
         */
        if let reason = data["reason"] as? String, reason != "null"{
            self.reason = reason
        }
        ...
        historyTitleValues.removeAll()
        if let history = data["history"] as? [[String:Any]] {
            for value in history{
                
                if let time = value["actionAt"] as? Double,let status = value["status"] as? String {
                    self.historyTitles.append(status)
                    self.historyTitleValues.append(String(time))
                }
            }
        }
    }
