...
protocol UpdatingHomePageSubscribePosts: class {
...
class PostedByViewController: UIViewController {
    ...
    var updatingHomePagePostsObject:UpdatingHomePageSubscribePosts?
...
//        self.navigationController?.navigationBar.isTranslucent = true
        ...
        if isChannel, let channelName = self.channelName{
            ...
            self.subscribeButtonOutlet.makeBorderWidth(width: 1.0, color: Utility.appColor())
        }else if isCategory{
    ...
    func setChannelDetails(){
        ...
        if let channelData = self.postedByViewModel.channelModel{
            ...
            if channelData.isSubscribed == 0{
                ...
                self.subscribeButtonOutlet.backgroundColor = Utility.appColor()
    ...
    @IBAction func subscribeButtonAction(_ sender: Any) {
        if var channelData = self.postedByViewModel.channelModel, let id = channelData.channelId{
            if self.subscribeButtonOutlet.isSelected{
                ...
                self.updatingHomePagePostsObject?.updateSubscribePosts(cellIndex: self.forYouPostSelectedCellIndex!, subscribe: 0)

            }else{
                channelData.isSubscribed = 1
                self.updatingHomePagePostsObject?.updateSubscribePosts(cellIndex: self.forYouPostSelectedCellIndex!, subscribe: 1)
        ...
        postedByViewModel.getPostedByHashTagData(isHashTag: isHashTag, hashTag: tagName, isChannel: isChannel) { (success, error, canServiceCall) in
            ...
            if success{
                ...
                /*
                 ...
                 Fixed By:- Nikunj C
                 Description of Fix:- if noofPostsCount is 0 then show postedByArray's count
                 */
                if self.noofPostsCount ?? 0 < self.postedByViewModel.postedByArray.count{
...
//            cell.setPostedByData(modelData: socialModelData)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if AppConstants.appType == .picoadda {
            ...
            if isFromProfileBtn {
                Route.navigateToAllPostsVertically(navigationController: self.navigationController, postId:self.socialModelArray[indexPath.row].postId,postsArray: nil,hashTagName: self.hashTagName, isCommingFromChat:true,isFromProfilePage:true,isFromTrendingPage:true)

            }else {
                Route.navigateToAllPostsVertically(navigationController: self.navigationController, postId:self.postedByViewModel.postedByArray[indexPath.row].postId,postsArray: nil,hashTagName: self.hashTagName, isCommingFromChat:true,isFromProfilePage:true,isFromTrendingPage:true)
            }
        }
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if scrollView == self.postedByCollectionView{
            ...
            self.navigationTitleLable.textColor = UIColor.setColor(lightMode: "292929", darkMode: "FFFFFF")
            if ratio > 0.2{
                self.navigationTitleLable.text = self.hashTagName
                self.navigationTitleLable.isHidden = false
                self.navigationView.backgroundColor = UIColor.setColor(lightMode: "FFFFFF", darkMode: "292929")
                self.navigationTitleLable.textColor = UIColor.setColor(lightMode: "292929", darkMode: "FFFFFF")
                   // UIColor.black
            }else{
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        header.coverImageView.setImageOn(imageUrl: self.hashTagImageUrl, defaultImage: Utility.setDefautlCoverImage())

        if let postsCount = noofPostsCount {
            if self.postedByViewModel.postedByArray.count > 1 {
                header.hashTagViewsCount.text = "\(postsCount) " + "Posts".localized
            }else{
                header.hashTagViewsCount.text = "\(postsCount) " + "Post".localized
            }
