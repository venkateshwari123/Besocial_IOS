//
//  SocialSubDetailsCollectionViewCell.swift
//  PicoAdda
//
...
import UIKit
protocol SocialSubDetailsCollectionViewCellDelegate {
...
class SocialSubDetailsCollectionViewCell: UICollectionViewCell {
    ...
    @IBOutlet weak var sendTiplbl: UILabel!
    ...
    var index: Int = 0
    var videoUrl: URL?
    var delegate: SocialSubDetailsCollectionViewCellDelegate?
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        self.sendTiplbl.text = "Send Tip".localized
...
//        self.likesCountLabel.text = "Liked by \(socialModel.likesCount) people"
        self.detailLinkHandler(socialModel: socialModel)
...
//                self.viewAllCommentsButton.setTitle("View all \(socialModel.commentCount) comments", for: .normal)
    ...
    private func detailLinkHandler(socialModel: SocialModel){
        postTitleLabel.customize { label in
            ...
            label.handleMentionTap {
                self.delegate?.hastagOrProfileSelected(byTag: "@" + $0, noOfPost: socialModel.noOfPosts ?? 0)
            }
...
//        }
        ...
        self.firstCommentLabel.userHandleLinkTapHandler = { [weak self] (label, string, range) in
            print(string)
            self?.delegate?.hastagOrProfileSelected(byTag: string, noOfPost: socialModel.noOfPosts ?? 0)
        }
