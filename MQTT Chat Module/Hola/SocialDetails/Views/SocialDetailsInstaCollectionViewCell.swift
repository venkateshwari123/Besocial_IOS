//
//  SocialDetailsInstaCollectionViewCell.swift
//  PicoAdda
//
...
protocol SocialDetailsInstaCollectionViewCellDelegate {
...
class SocialDetailsInstaCollectionViewCell: UICollectionViewCell {
    ...
    var index: Int = 0
    var videoUrl: URL?
    var delegate: SocialDetailsInstaCollectionViewCellDelegate?
    ...
    override func awakeFromNib() {
        ...
        /* Bug Name : Add Pinch to zoom
                 Fix Date : 26-May-2021
                 Fixed By : Jayaram G
                 Description Of Fix : added required gesture
                 */
    ...
    func setSocialCellData(socialModel: SocialModel){
        ...
        /* Bug Name : I guess we need to show tips in coins now and not $
                 Fix Date : 13-Apr-2021
                 Fixed By : Vishal D
                 Description Of Fix : removed $ sign
                 */
        if socialModel.channelName == "" {
            if let businessPostType = socialModel.businessProfileObj?.businessPostTypeName , businessPostType.count > 0 , businessPostType != "Regular"{
                 ...
                 */
                Helper.addedUserImage(profilePic: socialModel.profilePic, imageView: self.userImageView, fullName: ((socialModel.fullNameWithSpace ?? socialModel.fullNameWithOutSpace) ?? socialModel.userName) ?? "P")
                if socialModel.isStar == 1 {
    ...
    @IBAction func purchasePostTap(_ sender: Any) {
        ...
        delegate?.paidPostTap(modelData: data,index: index)
    }
...
extension SocialDetailsInstaCollectionViewCell: UIGestureRecognizerDelegate {
