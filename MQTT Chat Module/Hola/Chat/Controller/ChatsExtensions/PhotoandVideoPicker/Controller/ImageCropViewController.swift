...
public protocol ImageCropViewControllerDelegate: class {
    func cropViewController(_ controller: ImageCropViewController, didFinishCroppingImage image: UIImage, transform: CGAffineTransform, cropRect: CGRect)
...
open class ImageCropViewController: UIViewController {
    ...
    open override func viewDidLoad() {
        ...
        navigationItem.leftBarButtonItem = UIBarButtonItem(title: "Cancel".localized,
                                                           style: .done,
                                                           target: self,
                                                           action: #selector(ImageCropViewController.cancel(_:)))
        navigationController?.navigationBar.tintColor = UIColor.black
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Done".localized,
                                                            style: .done,
                                                            target: self,
                                                            action: #selector(ImageCropViewController.done(_:)))
        
        if self.toolbarItems == nil {
            let flexibleSpace = UIBarButtonItem(barButtonSystemItem: .flexibleSpace, target: nil, action: nil)
            let constrainButton = UIBarButtonItem(title: "Constrain", style: .plain, target: self, action: #selector(ImageCropViewController.constrain(_:)))
    ...
    @objc func constrain(_ sender: UIBarButtonItem) {
        ...
        let original = UIAlertAction(title: "Original".localized, style: .default) { [unowned self] action in
        ...
        let square = UIAlertAction(title: "Square".localized, style: .default) { [unowned self] action in
...
//            self.cropView?.cropAspectRatio = ratio
        ...
        let threeByTwo = UIAlertAction(title: "3 x 2".localized, style: .default) { [unowned self] action in
            self.cropView?.cropAspectRatio = 2.0 / 3.0
        }
        actionSheet.addAction(threeByTwo)
        let threeByFive = UIAlertAction(title: "3 x 5".localized, style: .default) { [unowned self] action in
            self.cropView?.cropAspectRatio = 3.0 / 5.0
        }
        actionSheet.addAction(threeByFive)
        let fourByThree = UIAlertAction(title: "4 x 3".localized, style: .default) { [unowned self] action in
        ...
        let fourBySix = UIAlertAction(title: "4 x 6".localized, style: .default) { [unowned self] action in
            self.cropView?.cropAspectRatio = 4.0 / 6.0
        }
        actionSheet.addAction(fourBySix)
        let fiveBySeven = UIAlertAction(title: "5 x 7".localized, style: .default) { [unowned self] action in
            self.cropView?.cropAspectRatio = 5.0 / 7.0
        }
        actionSheet.addAction(fiveBySeven)
        let eightByTen = UIAlertAction(title: "8 x 10".localized, style: .default) { [unowned self] action in
            self.cropView?.cropAspectRatio = 8.0 / 10.0
        }
        actionSheet.addAction(eightByTen)
        let widescreen = UIAlertAction(title: "16 x 9".localized, style: .default) { [unowned self] action in
        ...
        let cancel = UIAlertAction(title: "Cancel".localized, style: .default) { [unowned self] action in
