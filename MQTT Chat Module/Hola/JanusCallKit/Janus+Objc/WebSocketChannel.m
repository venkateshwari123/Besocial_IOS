    ...
    if ([janus isEqualToString:@"success"]) {
        ...
        if (handle == nil) {
            ...
            if (plugin[@"unpublished"] != nil) {
                JanusHandle *jHandle = feedDict[plugin[@"unpublished"]];
                if (jHandle) {
                    jHandle.onLeaving(jHandle);
                }
            }
...
- (void)createSession {
    ...
    NSDictionary *createMessage = @{@"janus": @"create",
...
- (void)publisherCreateHandle {
    ...
    jt.success = ^(NSDictionary *data) {
        ...
        [self subscriberCreateRoom: handle];
    };
    jt.error = ^(NSDictionary *data) {
       // Handle Error
        NSLog(@"Got error in creating publisherCreateHandle");
    };
    transDict[transaction] = jt;
    
    NSDictionary *attachMessage = @{@"janus": @"attach",
...
- (void)publisherJoinRoom : (JanusHandle *)handle {
    NSString *transaction = [self randomStringWithLength:12];
    NSDictionary *body = @{@"request": @"join",
                           @"room": _roomId,
                           @"ptype": @"publisher",
                           @"display": @"ios webrtc"};
    NSDictionary *joinMessage = @{@"janus": @"message",
                                  @"transaction": transaction,
                                  @"session_id":sessionId,
                                  @"handle_id":handle.handleId,
                                  @"body": body};
    
    [_socket send:[self jsonMessage:joinMessage]]; //commented
}

- (void)publisherCreateOffer:(NSNumber *)handleId sdp: (RTCSessionDescription *)sdp {
    NSString *transaction = [self randomStringWithLength:12];
    
    NSDictionary *publish = @{@"request": @"configure",
                              @"audio": @YES,
                              @"video": @YES,
                              @"audiocodec":@"opus",
                              @"videocodec":@"Vp8",
    ...
    NSDictionary *jsep = @{@"type": type,
                           @"sdp": [sdp sdp]};
    NSDictionary *offerMessage = @{@"janus": @"message",
                                   @"body": publish,
                                   @"jsep": jsep,
                                   @"transaction": transaction,
                                   @"session_id": sessionId,
...
- (void)trickleCandidate:(NSNumber *) handleId candidate: (RTCIceCandidate *)candidate {
    NSDictionary *candidateDict = @{@"candidate": candidate.sdp,
                                    @"sdpMid": candidate.sdpMid,
                                    @"sdpMLineIndex": [NSNumber numberWithInt: candidate.sdpMLineIndex]};
    
    NSDictionary *trickleMessage = @{@"janus": @"trickle",
...
- (void)trickleCandidateComplete:(NSNumber *) handleId {
    NSDictionary *candidateDict = @{@"completed": @YES};
    NSDictionary *trickleMessage = @{@"janus": @"trickle",
                                     @"candidate": candidateDict,
                                     @"transaction": [self randomStringWithLength:12],
                                     @"session_id":sessionId,
...
- (void)subscriberCreateRoom:(JanusHandle*)handle {
    NSString *transaction = [self randomStringWithLength:12];
    JanusTransaction *jt = [[JanusTransaction alloc] init];
    jt.tid = transaction;
    jt.success = ^(NSDictionary *data) {
        NSLog(@"Created New Room");
        NSLog(@"%@", data);
        [self publisherJoinRoom: handle];
        [self.delegate onCreatedRoom:handle.handleId];
    };
    NSDictionary *body = @{@"request": @"create",
                           @"room": _roomId,
                           @"is_private": @NO,
                           @"audiocodec":@"opus",
                           @"videocodec":@"Vp8",
    ...
    NSDictionary *message = @{@"janus": @"message",
                              @"transaction": transaction,
                              @"session_id": sessionId,
                              @"handle_id": handle.handleId,
                              @"body": body};
...
- (void)subscriberCreateAnswer:(NSNumber *)handleId sdp: (RTCSessionDescription *)sdp {
    NSString *transaction = [self randomStringWithLength:12];
    NSDictionary *body = @{@"request": @"start",
                           @"room": _roomId};
    NSString *type = [RTCSessionDescription stringForType:sdp.type];
    NSDictionary *jsep = @{@"type": type,
                           @"sdp": [sdp sdp]};
...
- (void)subscriberOnLeaving:(JanusHandle *) handle {
    ...
    jt.error = ^(NSDictionary *data) {};
    transDict[transaction] = jt;
    NSDictionary *message = @{@"janus": @"detach",
                              @"transaction": transaction,
                              @"session_id": sessionId,
                              @"handle_id": handle.handleId};
    [_socket send:[self jsonMessage:message]];
...
- (void)keepAlive {
    ...
    NSDictionary *dict = @{@"janus": @"keepalive",
                           @"session_id": sessionId,
                           @"transaction": [self randomStringWithLength:12]};
