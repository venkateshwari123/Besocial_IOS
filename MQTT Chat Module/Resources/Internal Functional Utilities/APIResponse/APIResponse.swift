...
//  GroupCalling
//
//  Created by Rahul Sharma on 23/11/19.
...
class APIResponse {
    ...
    var statusCode: HTTPStatusCode = .noResponse

    init(_ response: [String:Any], with status: HTTPURLResponse) {
        self.response = response
        
        if let statusCode = HTTPStatusCode(rawValue: status.statusCode) {
