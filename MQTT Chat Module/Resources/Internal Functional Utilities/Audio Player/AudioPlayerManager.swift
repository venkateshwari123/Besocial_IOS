...
open class AudioPlayerManager: NSObject {
	...
	open var bufferStrategy						= BufferStrategy.minimizeStalling {
		didSet {
			if #available(iOS 10.0, *) {
				self.player?.automaticallyWaitsToMinimizeStalling = self.bufferStrategy == .playImmediately
			}
		}
	}
	...
	fileprivate func setupAudioSession() {
        let _ = try? AVAudioSession.sharedInstance().setCategory(AVAudioSession.Category(rawValue: convertFromAVAudioSessionCategory(AVAudioSession.Category.playback)))
