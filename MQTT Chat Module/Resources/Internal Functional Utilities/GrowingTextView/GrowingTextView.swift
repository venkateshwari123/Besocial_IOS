...
//  Copyright © 2018 3Embed Software Tech Pvt Ltd. All rights reserved.
//
...
open class GrowingTextView: UITextView {
    ...
    open var heightConstraintObj: NSLayoutConstraint?
        ...
        for constraint in constraints {
            if (constraint.firstAttribute == .height) {
                if (constraint.relation == .equal) {
                    heightConstraintObj = constraint;
                }
            }
        }
    }
    ...
    override open func layoutSubviews() {
        ...
        if (heightConstraint == nil) {
            heightConstraintObj = NSLayoutConstraint(item: self, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1.0, constant: height)
            addConstraint(heightConstraintObj!)
        }
        
        // Update height constraint if needed
        if height != heightConstraintObj?.constant {
            shouldScrollAfterHeightChanged = true
            heightConstraintObj?.constant = height
...
//                ]
                var attributes: [NSAttributedString.Key: Any] = [
                    (convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor) as NSString) as NSAttributedString.Key: placeholderColor,
                    (convertFromNSAttributedStringKey(NSAttributedString.Key.paragraphStyle) as NSString) as NSAttributedString.Key: paragraphStyle
                ]
                
                if let font = font {
                    attributes[(convertFromNSAttributedStringKey(NSAttributedString.Key.font) as NSString) as NSAttributedString.Key] = font
                }
