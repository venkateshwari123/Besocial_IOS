        ...
        if let view = recognizer.view {
            if view is UIImageView {
                ...
                if recognizer.state == .began {
                    for imageView in subImageViews(view: drawingView) {
    ...
    @objc func tapGesture(_ recognizer: UITapGestureRecognizer) {
        if let view = recognizer.view {
            if view is UIImageView {
                ...
                if view == activeTextView && colorPickerView.isHidden{
                    colorsMainVeiwHeightConstraint.constant = 50
                    colorsCollectionViewHeightConstraint.constant = 40
                    colorPickerView.isHidden = false
                    fontTypeStackView.isHidden = false
                    self.activeTextView?.font = self.textFont
//                    self.activeTextView?.becomeFirstResponder()
    ...
    func moveView(view: UIView, recognizer: UIPanGestureRecognizer)  {
        ...
        view.center = CGPoint(x: view.center.x + recognizer.translation(in: drawingView).x,
                              y: view.center.y + recognizer.translation(in: drawingView).y)
        ...
        if let previousPoint = lastPanPoint {
            ...
            if deleteView.frame.contains(pointToSuperView) && !deleteView.frame.contains(previousPoint) {
                ...
                UIView.animate(withDuration: 0.3, animations: {
                    ...
                    view.center = recognizer.location(in: self.drawingView)
                })
            }
        ...
        if recognizer.state == .ended {
            ...
            } else if !drawingView.bounds.contains(view.center) { //Snap the view back to canvasImageView
                UIView.animate(withDuration: 0.3, animations: {
                    view.center = self.drawingView.center
                })
            }
            /*
             Bug Name:- icon below share or filter button is not accessable
             Fix Date:- 29th Oct 2021
             Fixed By:- Nikunj T
             Description of Fix:- move view little above
             */
            
            else if view.center.y > ( drawingView.bounds.height - 70) {
                UIView.animate(withDuration: 0.3, animations: {
                    view.center.y = self.drawingView.bounds.height - 85
                })
            }
        }
    }
