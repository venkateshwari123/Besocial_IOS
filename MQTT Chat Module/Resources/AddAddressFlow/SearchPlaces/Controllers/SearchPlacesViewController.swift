...
class SearchPlacesViewController: UIViewController {
    ...
    func setUI()
    {
        searchAddressField.configureSearchTextfied(color: UIColor.setColor(lightMode: "#242A4B", darkMode: AppColourStr.whiteColor), backgroundColor: UIColor.setColor(lightMode: "#F5F4F4", darkMode: "#272727"))
        self.searchAddressField.tintColor = .gray
        ...
        searchAddressField?.textField?.attributedPlaceholder = NSAttributedString(string: "Search Address".localized, attributes: [NSAttributedString.Key.font:UIFont(name: "Poppins-Regular", size: 12.0) ?? UIFont.boldSystemFont(ofSize: 14.0),NSAttributedString.Key.foregroundColor:Helper.getUIColor(color: "#BCBFD1")])
