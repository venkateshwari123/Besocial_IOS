...
+ (UIImage *)jsq_voiceImage{
    ...
    UIImageView *tempImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
    UIImage *updatedImage = [tempImageView.image jsq_imageMaskedWithColor:[UIColor labelColor]];
    if (@available(iOS 13.0, *)) {
        tempImageView.image = [tempImageView.image imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        [tempImageView setTintColor : [UIColor whiteColor] ];
        [tempImageView setImage: [UIImage imageNamed: @"record_icon"]];
